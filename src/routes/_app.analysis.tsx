...
function Analysis() {
  ...
  const stats = useMemo(() => {
    ...
    filtered.forEach((i) => {
      dow[new Date(i.created_at).getDay()].count++;
    });
    ...
    const pctChange =
      prevPeriod.length === 0 ? 100 : ((total - prevPeriod.length) / prevPeriod.length) * 100;

    return {
      total,
      avgRating,
      topCat: categories[0]?.name || "—",
      topLoc: locations[0]?.name || "—",
      categories,
      locations,
      dailySeries,
      ratingDist,
  ...
  const exportCSV = () => {
    ...
    a.href = url;
    a.download = `cases-analysis-${format(new Date(), "yyyy-MM-dd")}.csv`;
    a.click();
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex gap-2">
          <button
            onClick={exportCSV}
            className="px-3 py-2 text-[13px] bg-white border border-[#e0e4ed] rounded-md hover:bg-[#f4f6f9] text-[#5a6478] flex items-center gap-2 cursor-pointer transition-colors"
          >
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <div className="flex items-center gap-1">
            <input
              ...
              className="w-20 px-2 py-1.5 text-[12px] border border-[#e0e4ed] rounded-md focus:outline-none focus:border-[#0a1f44]"
            />
            <span className="text-[#5a6478]">-</span>
            <input
              type="number"
      ...
      <div ref={reportRef} className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          ...
          <KPICard
            ...
            value={
              <>
                ...
                <span className="text-[14px] ml-0.5">/ 5</span>
              </>
            }
            accent="#c47d00"
          />
          <KPICard
            label="Satisfaction"
            value={<AnimatedNumber value={Math.round(stats.satisfaction)} suffix="%" />}
            accent="#1a7a4a"
          />
          <KPICard
            label="Top Category"
            value={<span className="text-[15px] truncate">{stats.topCat}</span>}
            accent="#8b0000"
            small
          />
          <KPICard
            ...
            value={
              <span className={stats.pctChange > 0 ? "text-[#c0392b]" : "text-[#1a7a4a]"}>
                {stats.pctChange > 0 ? "+" : ""}
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          ...
          <Section
            ...
            subtitle={`${stats.satisfaction.toFixed(0)}% satisfaction rate`}
          >
              ...
              <table className="text-[11px] border-collapse w-full">
                <thead>
                  <tr>
                    ...
                    {stats.topLocs.map((l) => (
                      <th
                        key={l}
                        className="p-2 text-[#5a6478] font-medium text-center min-w-[80px] text-[10px]"
                      >
                        {l}
                      </th>
                    ))}
                ...
                <tbody>
                  {stats.topCats.map((c, ci) => (
                    <tr key={c}>
                      <td className="p-2 text-[#5a6478] font-medium text-right pr-3 text-[10px]">
                        {c}
                      </td>
            ...
            <table className="w-full text-[12px]">
              ...
              <tbody>
                {stats.categories.map((c, i) => (
                  <tr
                    key={c.name}
                    className="border-t border-[#e0e4ed] hover:bg-[#f8f9fc] transition-colors"
                  >
                    ...
                    <td className="px-3 py-2">
                      <div className="h-1.5 bg-[#f4f6f9] rounded-full overflow-hidden w-full min-w-[80px]">
                        <div
                          ...
                          style={{ width: `${c.pct}%`, background: colorFor(i) }}
                        />
                      </div>
                    </td>
                  </tr>
...
function KPICard({
  ...
  return (
    <div className="card-surface card-hover p-5 relative overflow-hidden">
      <div
        className="absolute top-0 left-0 w-1 h-full rounded-l-[12px]"
      ...
      <div
        className={`font-semibold text-[#0a1f44] ${small ? "text-[15px] leading-tight" : "text-[28px] tabular-nums leading-none"}`}
      >
        {value}
      </div>
    </div>
